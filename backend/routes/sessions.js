  ...
  try {
    ...
    if (session.requesterRating && session.recipientRating) {
      ...
      if (course) {
        ...
        if (user) {
          const skillIndex = user.skillsTeach.findIndex(
          ...
          if (skillIndex !== -1) {
            ...
            user.skillsTeach[skillIndex].rating = 
              (user.skillsTeach[skillIndex].rating * (user.skillsTeach[skillIndex].ratingCount - 1) + session.requesterRating) /
        ...
        course.averageRating = 
          (course.averageRating * (course.ratingCount - 1) + session.requesterRating) / course.ratingCount;
        await course.save();
      }
    }
